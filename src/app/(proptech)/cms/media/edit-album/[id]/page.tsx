...
import { webGalleryService, Gallery, GalleryPhoto } from '@/services/webGalleryService';
...
export default function EditAlbumPage() {
  ...
  useEffect(() => {
    const loadAlbum = async () => {
      try {
        setIsLoading(true);
        const gallery = await webGalleryService.getGalleryById(albumId);
        setTitle(gallery.title);
        setDescription(gallery.description || '');
        setIsPublished(gallery.isPublished || false);
  ...
  const removeExistingPhoto = async (photoId: number) => {
    ...
    try {
      await webGalleryService.deletePhoto(photoId);
  ...
  const handleSaveAlbum = async () => {
    ...
    try {
      ...
      await webGalleryService.updateGallery(albumId, {
        title: title.trim(),
      ...
      if (selectedFiles.length > 0) {
        ...
        for (const selectedFile of selectedFiles) {
          try {
            await webGalleryService.uploadPhoto(albumId, selectedFile.file);
            uploadedCount++;
        ...
        if (uploadedCount === totalFiles) {
          toast.success(`Álbum actualizado con ${uploadedCount} foto(s) nueva(s)`);
        } else {
          toast.warning(`Álbum actualizado, pero ${totalFiles - uploadedCount} foto(s) no se pudieron subir`);
        }
      } else {
      ...
      router.push('/cms/media?tab=albums');
    } catch (error: any) {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                Fotos Actuales del Álbum
              </h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {existingPhotos.length} foto(s) en el álbum. Haz clic en una foto para eliminarla.
              </p>
            </div>
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {selectedFiles.length === 0 
                ? 'Agrega nuevas fotos arrastrándolas o haciendo clic en el área de abajo'
                : `${selectedFiles.length} nueva(s) foto(s) seleccionada(s)`}
            </p>
          </div>
        ...
        <div className="sticky bottom-0 mt-6 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 -mx-4 rounded-t-lg shadow-lg sm:hidden">
          ...
          >
            {(isSaving || isUploading) ? (
              ...
              <>
                <CloudArrowUpIcon className="w-5 h-5" />
                Guardar Cambios ({selectedFiles.length} nuevas)
              </>
            )}
          </button>
        </div>
